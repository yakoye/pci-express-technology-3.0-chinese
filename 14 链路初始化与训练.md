## 14 链路初始化与训练（Link Initialization & Training）
...
### 本章
 ...
 本章讨论物理层链路训练与状态机（LTSSM, Link Training and Status State Machine）的行为，包括从上电或复位开始，到链路进入完全运行的 L0 状态，并开始正常报文传输的整个初始化过程。此外，本章还将讨论链路电源状态 L0s、L1、L2、L3 以及这些状态之间的转换。本章也会讨论链路恢复状态（Recovery）中，重新完成位锁定（Bit Lock）、符号锁定（Symbol Lock）或者块锁定（Block Lock）的过程。最后，还讨论了用于链路带宽管理的链路速率以及位宽转换。

### 下一章

下一章将讨论 PCIe 端口或者链路中存在的错误类型，如何检测、报告这些错误，以及这些错误的处理选项。因为 PCIe 设计中向后兼容了 PCI 的错误报告机制，所以将首先介绍 PCI 的错误处理机制作为背景知识。然后我们将关注于PCIe 对可纠正、非致命以及致命错误的处理机制。
...
- **符号锁定（Symbol Lock）**：对于 8b/10b 编码 (Gen1/Gen2) 而言，训练的下一步是进行符号锁定。同位锁定类似，虽然接收端目前可以识别单个比特，但是不知道由 10 个比特组成的符号的边界在哪里。当发送端和接收端交换训练有序集 TS1 和 TS2 时，接收端在比特流中搜索可识别的数据图样（pattern）。通过查找 COM 符号的位置可以较为简单地实现这一点。COM 符号独特的编码使得它很容易识别，识别 COM 字符后，接收方不但定位到了两个符号之间的边界，而且也能定位到两个有序集之间的边界（译注：因为有序集 TS1/TS2 以 COM 字符作为开端）。有关这方面的更多信息，参见第 396 页的 “实现符号锁定”。

- **块锁定（Block Lock）**：对于 8.0 GT/s（Gen3） 所使用的块锁定，处理过程与符号锁定略有不同，因为没有使用8b/10b 编码方案，所以也就不存在 COM 字符。然而，接收端仍然需要在接收比特流中找到可识别的报文边界。解决方案是在训练有序集中包含更多的电气空闲退出有序集（EIEOS，Electrical Idle Exit Ordered Set)，用于定位边界。可以通过查找 00h 和 FFh 字节交替的数据图样来找到 EIEOS ，并且 EIEOS 的位置定义了块之间的边界，因为根据协议，当该有序集结束后，下一个块必须紧随其后开始。
...
- **链路数据速率（Link Data Rate）**: 在复位后，链路初始化与训练状态机总是会先将数据速率设置为默认的 2.5 GT/s，以实现对第一代协议的向后兼容（第一代协议速率为  2.5 GT/s）。如果链路宣称可以实现更高的速率，那么在训练时双方会通告自己支持的最高速率，完成训练后，LTSSM 会自动重新进行一次过程更短的训练，将链路速率改变为双方都支持的最高速率。

- **通道间去偏斜（Lane-to-Lane De-skew）**： 各个通道之间的传输线长度差异或者其他因素，会导致多通道链路中原本同时传输的多个并行比特，到达接收方的时间有所差异，这被称之为信号偏斜（signal skew）现象。接收方需要通过延迟较早到达的通道，以对齐所有通道上的信号的到达时间，补偿通道之间信号传输快慢差异。在协议规定的可允许的信号偏移范围内，（对于 2.5 GT/s 速率来说，最大信号偏斜是 20ns)，接收方需要能够自动对偏移进行补偿。多通道间信号偏移补偿机制能够帮上 PCB 设计者一个大忙，因为他们不再需要设置复杂精细的约束，走出非常精确的等长信号传输线，有时候这是很难的。信号偏移补偿机制和极性翻转以及通道翻转机制一起，可以很大程度上简化高可靠的高速 PCIe PCB 设计。
...
从图中 14-4 中可以看到，TS1/TS2 由 16 个符号（译注：字节）组成。在 LTSSM 的轮询、配置以及恢复状态中，通信双方会交换 TS1/TS2 有序集，本文会在后续的 LTSSM 章节详细讨论，后文分别在表 14-1/14-2 中，用表格的形式描述了 TS1/TS2 16 个符号的具体含义。

为了便于理解和行文，以下用 Gen1 表示速率为 2.5 GT/s 的情况，Gen2 表示速率为 5.0 GT/s 的情况，Gen3 表示速率为 8.0 GT/s 的情况。另外值得注意的是，在较低速率下，Link 以及 Lane 字段中的填充字符（PAD character）使用 K23.7 ，而在 Gen3 中使用的是 F7h。我们后续的讨论中不会特别提及两者的差异，而统一表示为填充字符，因为这不重要。
...
|1 | Link Number 链路编号<br> • 不支持 Gen3 的端口: 0‐255, PAD<br> •支持 Gen3 的下游端口: 0‐31, PAD<br> •支持 Gen3 的上游端口: 0‐255, PAD<br> |
...
|5 | Training Control (0=De‐assert, 1 = Assert) 训练控制字段（1 表示使能）<br> • Bit 0 — Hot Reset<br> • Bit 1 — Disable Link<br> • Bit 2 — Loopback<br> • Bit 3 — Disable Scrambling (2.5 或 5.0 GT/s; Gen3 中为保留字段)<br> • Bit 4 — Compliance Receive (2.5 GT/s 速率可选支持, 其他速率必须支持)<br> • Bits 7:5 — 保留字段, 设置为 0<br> |
...
|9 | Gen1/Gen2：<br/> • TS1 标识符(4Ah) ，即 D10.2<br/>Gen3:<br> • Bits 5:0 — Post‐cursor Coefficient.<br> • Bit 6 — Reject Coefficient Values. <br>仅在 Recovery.Equalization  LTSSM 状态的特定阶段中使用, 其他状态中为必须为 0<br> • Bit 7 — Parity (P) <br/>这是覆盖符号 6，7，8 以及 符号 9[6:0] 的校验码<br>接收方必须重新计算校验位并与接收到的校验位比较<br>只有比较结果通过的 TS1 才是有效的<br> |
...
读者可能会对符号 6 中的 EQ TS1 部分感到疑惑：既然只有 8.0 GT/s 速率支持均衡，为什么较低的速率也会用到 EQ TS1？ 它们其实是给那些支持 Gen3 但是当前工作在较低速率，并且想切换到 8.0 GT/s 的通道传递 EQ 值用的。关于这部分的更多细节，以及 Gen3 均衡过程相关的内容，可以参阅本章 “链路均衡概览” 一节，原文 577 页。
...
| 5      | Training Control (0=De‐assert, 1 = Assert) 训练控制字段（1 表示使能）<br> • Bit 0 — Hot Reset<br> • Bit 1 — Disable Link<br> • Bit 2 — Loopback<br> • Bit 3 — Disable Scrambling (2.5 或 5.0 GT/s; Gen3 中为保留字段)<br> • Bits 7:4 — 保留字段, 设置为 0<br> |
| 6      | Gen1/Gen2:<br> • TS2 标识符（45h），即 D5.2<br> • 仅在 EQ TS2 中使用，普通 TS2 不适用<br> Bits 2:0 — Receiver preset hint<br> Bits 6:3 — Transmitter Preset<br> Bit 7 — Equalization Command<br> Gen3:<br> • Bits 5:0 — 保留字段 <br/>仅在 Recovery.Equalization  以及 Loopback 等 LTSSM 状态中使用, 其他状态中为必须为 0<br/> • Bit 6 —  Quiesce Guarantee.<br/> 仅在 Recovery.RcvrCfg 等 LTSSM 状态中使用, 其他状态中为保留字段<br/> • Bit 7 —  Request Equalization.<br/> 仅在 Recovery.RcvrCfg 等 LTSSM 状态中使用, 其他状态中为保留字段<br/> |
...
- **Symbol 3 (N_FTS)**： 表示当前链路速率下，接收方从 L0s 电源状态退出返回 L0 状态所需要接收的快速训练序列（FTS）数量。在退出 L0s 状态时，发送方至少会发送 N_FTS 个 FTS。这一过程所需的时间取决于所需 FTS 的数量以及当前的链路速率。举例来说，2.5 GT/s 速率下，每个符号的发送时间为 4ns，所以如果需要 200 个 FTS，那么就是 200 FTS * 每个 FTS 4 个符号 * 4ns = 3200 ns。如果发送方置位了扩展同步比特，那么将发送 4096 个 FTS，发送这么多 FTS 是为了给外部链路观测工具建立比特同步和符号同步留出足够的时间，因为有些外部工具同步所需的时间比较长。
- **Symbol 4 (Rate ID):** 设备报告其所支持的数据速率，以及一些提供给由硬件发起的带宽改变功能的信息。所有设备必须支持 2.5 GT/s 速率，并且链路在复位后始终会自动被训练为 2.5 GT/s 速率，所以任何新的设备都需要向后兼容这个速率。如果设备支持 8.0 GT/s 速率，那么它也需要支持 5.0 GT/s 速率。此外，这个符号中还包括的其他信息有：
  ...
  - **Selectable De‐emphasis**
    - **上游端口（Upstream Ports）**设置该域表示他们在 5.0 GT/s 速率下期望的去加重水平设置，具体的预期设置取决于实现。在 Recovery.RcvrCfg 状态中，设备锁存接收到的该域的值，存放在设备内部。（协议描述中，这个值会被保存在一个名为 select_deemphasis 的变量中）
  ...
  -  **Link Upconfigure Capability**：该域表示了一个较宽的链路在减少链路宽度后，是否有能力重新恢复原先较宽的链路宽度。如果在 Configuration.Complete 状态中，双方设备都报告自己具有这个能力，那么就作为一项共识记录下来。（比如将内部的一个 upconfigure_capable 比特置位）

- **Symbol 5 (Training Control):** 交流链路双方的一些特殊情况，比如进行一次热复位（Hot Reset），使能回环模式，关闭链路或者关闭加扰。
- **Symbols 6‐9 (Equalization Control):** 
  - Gen1/Gen2，符号 7-9 只是 TS1 或者 TS2 的标识符，符号 6 通常也只是标识符，此时符号 6 比特 7（Use Preset 域） 为 0 。但是，如果这一比特为 1，那么代表当前的有序符号集是一个下游端口（DSP，面向下游的端口，比如 RC 的端口）发送的 EQ TS1 或者 EQ TS2。其中 ”EQ“ 代表均衡（Equalization），发送 EQ TS1/TS2 意味着链路速率将被提高为 8.0 GT/s，此时上游端口（USP，面向上游的端口，比如 EP 的端口）需要知道当前使用的均衡参数。在 EQ TS1 和 TS2 中，符号 6 提供给 USP 的信息包括发送端预设集选择（Preset）以及接收端预设集选择提示（Preset Hint）。支持 8.0 GT/s 速率的端口需要能够接收任意的 TS 类型（包括普通 TS 以及 EQ TS），但是如果端口不支持 8.0 GT/s 速率，则不需要接收 EQ TS 的能力。可能的预设集数值列在原文 579 和 580 页的表 14-8 和表 14-9 中。
...
- **Symbols 14-15 （DC Balance）:**   
  - Gen1/Gen2，只是 TS1 或者 TS2 的标识符，因为 DC 均衡已经由 8b/10b 编码完成。
...
### 14.3 链路训练与状态机（LTSSM，Link Training and Status State Machine）
...
此外，软件可以直接使链路进入一些其他状态，比如禁用状态（Disabled），回环状态（Loopback State）（Loopback）或者热复位（Hot Reset）状态。这里，这些状态被归纳为**其他状态**。
...
#### 14.3.2 LTSSM 状态概要（链路训练与状态机状态概要）
...
- **轮询状态 Polling**：在轮询状态中，发送方将以 2.5 GT/s 的速率向对端发送 TS1 以及 TS2 序列，使用协议最低速率以实现对早期协议的向后兼容。接收端可以使用接收的 TS1/TS2 序列实现以下功能：
...
- **配置状态 Configuration**：上游（Upstream）和下游（Downstream）器件将分别按照他们上下游的角色，以 2.5 GT/s 速率，交换 TS1 和 TS2 序列，来实现以下的目标：
...
- **L0 状态**：L0 是一个链路全功能正常运行的状态，此时链路上会进行正常的 TLP、DLLP 报文和有序集的交换。L0 状态下，链路的速率可以比 2.5 GT/s 更高，但只能在进入 Recovery 状态，经历一次链路速率变化程序之后，才能切换到更高的速率。
...
- **回环状态（Loopback State） Loopback**：回环状态（Loopback State）定义为一种测试状态，但是协议中没有详细定义接收方的行为（比如接收方哪些逻辑会参与回环测试）。回环状态（Loopback State）中基本的行为很简单：回环发起方（Loopback Master）发送 TS1 有序集给回环接收方（Slave），并置位 TS1 训练控制字段（Training Control）中的回环（Loopback）比特。当接收方连续接收 2 个 回环比特置起的 TS1 序列后，进入回环状态（Loopback State），将接收到的任何内容都重新发送给发起方。发起方会检查接收到的内容，和先前发送的内容进行比较，如果内容经过双方的 8b/10b 编码（解码）后校验一致，说明链路可以通过环路验证，完整性没有问题。

- **禁用状态 Disable**：Disable 状态（Disable State）允许将链路配置为禁用状态，该状态下发送逻辑为电气空闲状态，接收逻辑为低阻状态。在某些情况下，比如链路状态变得不可靠，或者对端设备被意外移除，此时 Disable 是表示这些意外情况的必要状态。另外，软件也可以通过链路控制寄存器（Link Control Register）中的禁用比特（Disable bit），将设备配置为禁用状态。设备被禁用后，会连续发送 16 个链路禁用比特（Disable Link bit）置位的 TS1 序列，位于 TS1 的训练控制域（Traning Control Field），通知接收方进入禁用状态。

- **热复位状态（Hot Reset State） Hot Reset**：软件可以通过置位桥控制（Bridge Control）寄存器的配置次级总线复位（Secondary Bus Reset）比特，复位链路。软件配置后，桥的下游端口会发送训练控制域（Traning Control Field）热复位（Hot Reset）比特置位的 TS1 序列。（详情可见原文 837 页的 “Hot Reset (In-band Reset)” 节）当接收方接收到两个 TS1 序列后，必须复位自身设备。
...
- 目标数据速率为 Gen1 的 2.5 GT/s。如果进入该状态时的速率不是  2.5 GT/s，LTSSM 必须要在该次状态内等待 1ms，然后再将速率切换至 Gen1。
...
一旦链路双方的共模电压稳定在发送余量（Transmit Margin）域中允许的范围内之后，发送方会发送至少 1024 个连续的 TS1 有序集。因为链路双方退出检测状态的时刻可能不同，所以 TS1 命令序列的交换不是同步的。在 Gen1（2.5 GT/s） 速率下，发送 1024 个 TS1 序列花费 64us 。

Active 次状态下，有一些值得注意的细节：

- 必须在 TS1 序列的通道（Lane）和链路编号（Link number）字段中使用填充字符
- 设备需要通告（advertise）自己支持的所有速率，哪怕设备并不打算使用这些速率
- 接收方使用接收到的 TS1 序列实现比特锁定和符号锁定（低于 8.0 GT/s 的速率）或者块锁定（8.0 GT/s 速率）
...
现今，LTSSM 在复位之后会总是会将速率训练为 2.5 GT/s，哪怕支持更高的速率。在 LTSSM 状态机进入 L0 状态之后，一旦有更高的可用速率，状态机进入 Recovery 状态，并且尝试将速率训练为双方都支持的最高速率。双方可支持的最高速率会在双方交换的 TS1 和 TS2 序列中体现，所以任意一方都可以通过使自身的状态机进入 Recovery 状态，发起一次速率改变行为。协议中仍然会罗列 Polling.Speed 次状态，但会将其标注为不会进入（unreachable）的状态。

##### 14.5.1.3 Polling.Compliance

Polling.Compliance 次状态仅用于测试。发送方在该状态下发送特定测试码型（pattern），用于构造接近最差情况的码间干扰（ISI，Inter-Symbol Interference）和串扰（crosstalk）条件，便于分析链路信号质量。在该次状态中，可以发送两类码型：**Compliance Pattern** 和 **Modified Compliance Pattern**。

###### 用于 8b/10b 编码的 Compliance Pattern

该码型由 4 个符号（Symbol）按顺序重复组成，分别是：`K28.5-`、`D21.5+`、`K28.5+` 和 `D10.2-`。其中 `-` 表示负的当前运行差异（CRD，Current Running Disparity），`+` 表示正的 CRD。由于该码型会强制指定 CRD，所以在码型开始处允许出现 disparity error。

如果链路由多条通道组成，那么还会注入 4 个延迟符号（Delay Symbol）。表中的 `D` 不是普通的 D 字符，而是 Delay Symbol 的占位标记，实际是额外插入的 `K28.5` 符号。Delay Symbol 首先插入到 Lane 0：其中 2 个位于下一组 Compliance Pattern 之前，2 个位于该组 Compliance Pattern 之后。Lane 0 上最后一个 Delay Symbol 发送完成后，再以同样方式转移到 Lane 1；随后依次传播到 Lane 7，之后再回到 Lane 0 重新开始。每 8 条通道为一组重复该行为。

这种让 Delay Symbol 在相邻 Lane 之间移动的方式，可以增加相邻通道之间的干扰，从而形成更适合测试的接近最差情况的信号条件。

表 14-3 及其续表：8b/10b 编码的 Compliance Pattern
...
用于 128b/130b 编码的 Compliance Pattern 由 36 个 Block 组成，并按如下顺序循环重复：

1. 第 1 个 Block：同步头（Sync Header）为 `01b`，未加扰载荷由 64 个 `1` 后跟 64 个 `0` 组成。
2. 第 2 个 Block：同步头为 `01b`，未加扰载荷如表 14-4 所示。
3. 第 3 个 Block：同步头为 `01b`，未加扰载荷如表 14-5 所示。
4. 第 4 个 Block：EIEOS Block。
5. 后续 32 个 Data Block：每个 Block 包含 16 个经过加扰的 IDL 符号（`00h`）。

表 14-4 和表 14-5 中的码型每 8 条 Lane 后重复一次。`P` 表示当前使用的 4-bit 发送端 Preset 编码，`~P` 表示该 Preset 编码的逐位取反值。

表 14-4：128b/130b Compliance Pattern 的第 2 个 Block
...
表 14-5：128b/130b Compliance Pattern 的第 3 个 Block
...
Modified Compliance Pattern 在 Compliance Pattern 的基础上增加了错误状态字段，用于报告设备在 Polling.Compliance 状态下检测到的 Receiver Error 数量。

在 8b/10b 模式中，原始 Compliance Pattern 仍然会被使用，但会增加 2 个用于报告错误状态的符号。之所以使用 2 个符号而不是 1 个，是为了避免破坏该序列所要求的 disparity 关系。随后还会在末尾增加 2 个 `K28.5` 符号，因此基本码型总长度变为 8 个符号。

表 14-6：8b/10b Modified Compliance Pattern 的符号序列
...
`ERR[6:0]` 是 Receiver Error Count，用于表示 Pattern Lock 置位后检测到的错误数量。`ERR[7]` 是 Pattern Lock 指示位，表示接收端已经锁定到输入的 Modified Compliance Pattern。

该码型的 Delay 序列也与普通 Compliance Pattern 不同：它会在序列开头连续增加 4 个 `K28.5` 符号（表中用 `D` 表示），并在 8-symbol 基本码型末尾增加 4 个 `K28.7` 符号，因此在 Delay Pattern 转移到下一条 Lane 之前，总共会发送 16 个符号。与普通 Compliance Pattern 类似，基础码型会在相邻通道之间移动，以形成更合适的测试条件。

###### 用于 128b/130b 编码的 Modified Compliance Pattern

用于 128b/130b 编码的 Modified Compliance Pattern 由 65792 个 Block 组成，并按如下结构循环：

1. 1 个 EIEOS Block。
2. 256 个 Data Block，每个 Data Block 包含 16 个经过加扰的 IDL 符号（`00h`）。
3. 后续 255 组序列，每组包括：
   - 1 个 SOS；
   - 256 个 Data Block，每个 Data Block 包含 16 个经过加扰的 IDL 符号。

由于这些 Data Block 的载荷全为 0，最终链路上看到的主要就是对应 Lane 的加扰器输出。需要注意，扰码器不会因为 Sync Header 比特而推进，并且会由 EIEOS 初始化。由于扰码器种子值依赖于 Lane 编号，因此 Lane 编号必须被正确理解。

如果链路先完成了训练，随后软件通过设置 Link Control 2 寄存器中的 Enter Compliance 比特使 LTSSM 进入 Polling.Compliance，那么应使用训练期间已经分配好的 Lane 编号以及极性反转结果。如果某条 Lane 在训练期间并未激活，或者以其他方式进入该次状态，则使用端口默认分配的 Lane 编号。

此外，该码型中的 Data Block 并不构成 Data Stream，因此不需要满足 Data Stream 的相关要求，例如不需要发送 SDS Ordered Set，也不需要发送 EDS Token。

在 128b/130b 模式中，错误状态不再像 8b/10b 模式那样以单独 ERR Symbol 的形式突出显示，而是包含在 SOS 中。SOS 的最后 2 个字节用于报告 Polling.Compliance 期间的 Receiver Error Count。

###### 进入 Polling.Compliance 状态

与进入 Polling.Active 时类似，进入 Polling.Compliance 后，Link Control 2 寄存器中的 Transmit Margin 字段会用于设置本次状态下发送端使用的电压范围。

数据速率以及去加重（De-emphasis）/Preset 的选择方式如下：

- 如果端口只支持 2.5 GT/s，那么数据速率固定为 2.5 GT/s，去加重等级固定为 -3.5 dB。
- 否则，如果进入该次状态是因为接收到 8 个连续 TS1，并且这些 TS1 的 Compliance Receive 比特为 1b、Loopback 比特为 0b（分别为 TS1 Symbol 5 的 bit 4 和 bit 2），则数据速率选择任意 Lane 上双方都支持的最高速率。`select_deemphasis` 变量必须与 TS1 Symbol 4 中的 Selectable De-emphasis 比特匹配。如果选择的速率是 8.0 GT/s，则每条 Lane 的 `select_preset` 变量来自连续 TS1 中 Symbol 6 的发送端 Preset 信息；对于没有收到 8 个包含 Transmitter Preset 信息的连续 TS1 的 Lane，可自由选择任意支持的 Preset。
- 否则，如果 Link Control 2 寄存器中的 Enter Compliance 比特为 1b，则 Compliance Pattern 按 Target Link Speed 字段给定的速率发送。如果目标速率是 5.0 GT/s，当 Compliance Preset/De-emphasis 字段等于 `0001b` 时，`select_deemphasis` 变量置位；如果目标速率是 8.0 GT/s，则每条 Lane 的 `select_preset` 变量清为 `0b`，发送端必须使用 Compliance Preset/De-emphasis 字段指定的 Preset，前提是该字段不是保留编码。
- 最后，如果以上条件都不满足，则数据速率、Preset 和 De-emphasis 会按照组件支持的最高速率以及以这种方式进入 Polling.Compliance 的次数循环变化。第一次进入时使用 Setting Number 1，之后每重新进入一次就递增一个设置；如果重新进入超过 14 次，则循环重复该序列。这样可以方便地依次测试组件支持的各类发送设置：进入 Polling.Compliance 测试当前设置，回到 Polling.Active，再进入 Polling.Compliance 测试下一项设置。

表 14-7：Compliance 发送设置序列
...
如果最终选择的数据速率不是 2.5 GT/s，还需要额外满足以下要求：

- 如果 Polling.Active 中曾经发送过 TS1，则发送端在进入 Electrical Idle 之前必须发送 1 个或 2 个连续 EIOS。
- 如果 Polling.Active 中没有发送过 TS1，则发送端可以不发送 EIOS，直接进入 Electrical Idle。
- Electrical Idle 持续时间必须大于 1 ms 且小于 2 ms。在此期间，端口切换并稳定到新的数据速率。如果速率为 5.0 GT/s，De-emphasis 等级由 `select_deemphasis` 变量决定：`0b` 表示 -3.5 dB，`1b` 表示 -6.0 dB。如果速率为 8.0 GT/s，则由 `select_preset` 变量决定发送端 Preset。

###### Polling.Compliance 状态期间

确定数据速率以及 De-emphasis/Preset 后，Polling.Compliance 状态中的行为遵循以下规则。

**发送 Compliance Pattern**

如果进入 Polling.Compliance 不是因为 TS Ordered Set 中 Compliance Receive 比特为 1b 且 Loopback 比特为 0b，也不是因为 Link Control 2 寄存器中的 Enter Compliance 和 Enter Modified Compliance 两个比特同时置位，那么发送端会在所有已检测到的 Lane 上发送 Compliance Pattern。

退出到 Polling.Active 的条件如下：

1. 任意已检测到 Lane 的接收端检测到退出 Electrical Idle，并且 Enter Compliance 比特为 0b。
2. 自进入 Polling.Compliance 以来，Enter Compliance 比特已经被清为 0b。
3. 对于上游端口（Upstream Port），Enter Compliance 比特为 1b，且任意 Lane 上检测到 EIOS。该条件也会使 Enter Compliance 比特清为 0b。

如果进入 Polling.Compliance 时数据速率不是 2.5 GT/s，或者进入时 Enter Compliance 比特为 1b，那么发送端在转换到 Polling.Active 之前，需要发送 8 个连续 EIOS 并进入 Electrical Idle。在 Electrical Idle 期间，端口切换回 2.5 GT/s，并使用 -3.5 dB 去加重，持续时间应在 1 ms 到 2 ms 之间。发送多个 EIOS 的目的，是提高对端至少检测到一个 EIOS 并退出 Polling.Compliance 的概率，尤其是在通过 Enter Compliance 比特进入该状态时。

**发送 Modified Compliance Pattern**

如果进入 Polling.Compliance 是由 TS1 指示触发，并且 Compliance Receive 比特为 1b、Loopback 比特为 0b，或者 Link Control 2 寄存器中的 Enter Compliance 与 Enter Modified Compliance 比特同时置位，则发送端会在所有已检测到的 Lane 上发送 Modified Compliance Pattern，并且初始时将错误状态符号清为全 0。

当速率为 2.5 GT/s 或 5.0 GT/s 时：

- 每条 Lane 在看到一个 Modified Compliance Pattern 实例后，即可认为已经成功锁定到输入码型，并在自己回发的 Modified Compliance Pattern 中置位 Pattern Lock 位（即 8-bit 错误状态符号的 bit 7）。
- 错误状态符号不能参与锁定过程，因为在对端尚未锁定之前，这些符号本身的含义可能尚未定义。
- 一个码型实例指前文所述的 4 个符号序列：`K28.5`、`D21.5`、`K28.5`、`D10.2`，或者这些符号的补码形式（表示极性反转）。
- 被测设备必须在收到对端 Modified Compliance Pattern 后 1 ms 内，在自己发送的 Modified Compliance Pattern 中置位 Pattern Lock 位。
- 某条 Lane 上每出现一个接收错误，该 Lane 的错误计数加 1；计数达到 127 后饱和，不再继续增大，也不会回绕。

当速率为 8.0 GT/s 时：

- 进入该次状态时，`Error_Status` 字段设置为 `00h`。
- 被测设备必须在收到对端 Modified Compliance Pattern 后 4 ms 内，在自己发送的 Modified Compliance Pattern 中置位 Pattern Lock 位。
- 每条 Lane 在完成 Block Alignment 后独立置位 Pattern Lock。此后，Data Block 中的符号应为 IDL（`00h`）；任何不匹配的符号都会使错误计数加 1。Receiver Error Count 在 127 处饱和，并通过该码型中 SOS 的最后 2 个符号发送。
- Modified Compliance Pattern 的加扰规则与普通情况相同：扰码器种子值按 Lane 设置，由 EIEOS 初始化，SOS 不推进 LFSR。
- 规范提示，设备在获得 Block Alignment 前应等待足够长时间，以确保接收端已经稳定并且不会出现 bit slip；在置位 Pattern Lock 位之前，也可以重新验证 Block Alignment。

退出到 Polling.Active 的条件如下：

- 如果进入 Polling.Compliance 时 Enter Compliance 比特为 1b，并且之后 Enter Compliance 比特被清为 0b；或者对于上游端口，在任意 Lane 上接收到 EIOS，则退出到 Polling.Active，同时 Enter Compliance 比特清为 0b。
- 如果进入 Polling.Compliance 时数据速率不是 2.5 GT/s，或者进入时 Enter Compliance 比特为 1b，那么发送端在转换到 Polling.Active 前发送 8 个连续 EIOS 并进入 Electrical Idle。在 Electrical Idle 期间，端口切换回 2.5 GT/s、-3.5 dB De-emphasis，并保持 1 ms 到 2 ms。

退出到 Detect State 的条件如下：

- 如果 Link Control 2 寄存器中的 Enter Compliance 比特被清为 0b，并且设备被指示退出该次状态，则退出到 Detect State。


### 14.6 配置状态（Configuration State）

设备初始化时，Configuration 状态在 2.5 GT/s 速率下配置链路以及通道编号。5.0 GT/s 和 8.0 GT/s 速率时，设备也可能从 Recovery 状态进入 Configuration 状态。此时状态转换的主要目的是为了进行多通道设备的链路位宽动态转换。动态转换仅支持  5.0 GT/s 和 8.0 GT/s 速率的设备。这些高速率设备详细的状态转换过程将在原文 552 页开始的 “详细 Configuration 次状态” 一节中讨论。
...
5. 通道顺序颠倒是一项双方端口可选的特性。如果 USP 检测到通道编号颠倒，并有能力颠倒自己通道的逻辑编号，那么 USP 会在内部颠倒自己通道的逻辑编号，并返回通道编号不变的 TS1。这样一来，DSP 都不会意识到这个问题的存在。如果 USP 不支持这项功能，那么 DSP 会发现其接收到的 TS1 的通道编号相比它发送的 TS1 是颠倒的。如果 DSP 支持这项特性，那么它会纠正自身通道逻辑编号的顺序，并在 TS2 中发送纠正后的通道编号作为确认。
...
*转移至 Disable 状态（Disable State）*

如果端口受到更高层逻辑的指示，发送 Disable Link 比特置位的 TS1 或者 TS2，那么将转移至 Disable 状态（Disable State）。一般来说这个过程是由 DSP 发起的，但是在可选支持 Crosslink 的场景中，这可能是由 USP 发起的。DSP 在收到连续两个 Loopback 比特置位（译注：原文如此，应该是 Disable 比特置位）的 TS1 后，下一个状态将跳转为 Disable 状态（Disable State）。
...
如果下列条件中任意一项满足，那么转移至 Disable 状态（Disable State）：
...
如果 idle_to_rlock_transitioned 变量小于 FFh，并且当前数据速率为 8.0 GT/s，那么接下来转入 Configuration.Idle 状态。
...
第一种情况是链路双方都支持 2.5 GT/s 以上的速率，并且链路处于活跃状态时（数据链路层报告 DL_Active），或者链路一方通过 TS1/TS2 有序集请求速率转换时。举个例子，如果对方声明支持更高的速率，并且软件通过使能重训练（Retrain Link）比特，并将目标速率（Target Link Speed，见原文 569 页的图 14-26）设置为与当前不同的速率，那么 DSP 会发起一次速率转换。

第二种速率切换的情况是链路双方都支持 8.0 GT/s 速率，并且一方打算进行 Tx 均衡。这两种情况下变量 directed_speed_change 都会被设置为 1b，并且 changed_speed_recovery 比特会被清除为 0b。
...
第二种情况，在链路以 2.5 GT/s 速率训练完成，进入 L0 状态后，如果双方都支持更高的速率，那么 LTSSM 会进入 Recovery 状态，将速率调整为双方同时支持或者通告的最高速率。这种情况下，LTSSM 必须重新进行比特锁定和符号或者块锁定，以及链路间的去除偏斜操作。
...
在 Recovery.RcvrLock 状态中，面向上游端口（USP）可以指定面向下游端口（DSP）在 5.0 GT/s 速率下使用的去加重等级系数（de‐emphasis level）。USP 通过 TS1 序列中的可选去加重等级（Selectable  de‐emphasis leve）比特，来指定 DSP 的数值设置。但是 DSP 有可能会因为链路上的比特错误，无法接收到 USP 发送的这一信息，所以 USP 可以在因为速率切换进入  Recovery 状态后，再次指定 DSP 的去加重等级。如果 DSP 打算采用 USP 所要求的的数值，那么 DSP 必须在 Recovery.RcvrLock 状态中记录接收到的可选去加重等级比特的数值。
...
DSP 如果想将速率切换到 8.0 GT/s 并且重新进行均衡的话，需要发送  speed_change 比特置为 1，并且速率声明为 8.0 GT/s 的 EQ TS1 序列。如果一个 USP 连续接收到 8 个  speed_change 比特置为 1，并且速率声明支持 8.0 GT/s 的EQ TS1 或者 EQ TS2 序列后，USP 也会宣告支持使用 8.0 GT/s 速率，除非 USP 确认在该速率下存在可靠性问题，并且无法靠均衡解决。值得注意的是，任意端口可以在进入  Recovery 状态时，改变其通告支持的速率，但需要确保自身可以稳定可靠地支持这一速率。除了上述这种情形外，端口在 Recovery.RcvrLock 状态，Recovery.RcvrCfg 状态以及 Recovery.Equalization 状态中，不得改变自己通告的支持速率。
...
当数据速率为 8 GT/s 时，数据通道必须采取合适的均衡参数，以获得良好的信号完整性。本节的内容并不适用于更低速的速率。对于运行在 8.0 GT/s 速率的链路来说，他们并不是每次进入 Recovery  状态时，都需要经过 Recovery.Equalization 次状态。链路只会在满足下述情况时，才会进入 Recovery.Equalization 状态：
...
如果没有发起速率改变请求，（即 directed_speed_change 变量为 0，并且 TS1/TS2 中的 speed_change 比特也为 0），或者双方协商后发现彼此同时支持的最高速率只是 2.5 GT/s，此时 LTSSM 会转移至 Configuration 状态。
...
首先，链路会以  2.5 GT/s 的速率自动训练为 L0 状态。（这一行为很大概率会在后续的协议版本中保留，因为后续协议将提供和先前设备的向后兼容性。）
...
图 14-28  8.0 GT/s 速率 TS1/TS2 中的 EC 字段示意
...
如果链路速率是 2.5 GT/s 或者 8.0 GT/s，那么在进入该次状态前必须发送一个 EIOS，如果速率是 5.0 GT/s，那么必须发送两个。所谓进入电气空闲状态是指在链路上看到一些 EIOS，或者检测到，或者推断出当前正处于电气空闲状态（详见原文 736 页 ”Elecitrical Idle“ 节）。

只有当接收端也进入电气空闲状态后，才能进行链路运行速率改变。如果链路已经工作于双方所能支持的最高速率，那么进入该次状态也不会改变运行速率。

如果双方协商的新速率是 5.0 GT/s，那么去加重水平必须基于当前内部变量 select_deemphasis 的数值来决定。如果变量数值为 0b，那么采用 -6dB 的去加重水平，如果变量为 1b，那么转而采用 -3.5dB 的去加重水平。
...
| Recovery.RcvrCfg                                   | 1280 UI 内没有见到 TS1/TS2                   | 1280 UI 内没有见到 TS1/TS2                   | 4ms 内没有见到 TS1/TS2                       |
| Recovery.Speed (successful_speed_negotiation = 1b) | 1280 UI 内没有见到 TS1/TS2                   | 1280 UI 内没有见到 TS1/TS2                   | 4680UI 内没有见到 TS1/TS2                    |
...
否则，新的速率会恢复为 2.5 GT/s，并且  changed_speed_recovery 变量同样会保持为 0b。协议指出该情况对应于此前在 L0 状态中链路速率大于 2.5 GT/s，但是链路一方不能在该速率正常运行，并在第一次的 Recovery.RcvrLock 状态中超时退出。
...
- 链路双方支持的共同最高速率高于  2.5 GT/s，或者链路的速率已经高于  2.5 GT/s。
...
还有一种跳转到 Recovery.Speed 状态的情况是：链路双方自从 L0 或者 L1 状态进入 Recovery 状态后，链路双方*没能*将链路速率协商为更高的速率（changed_speed_recovery = 0b），不过此时链路的速率高于  2.5 GT/s，但是在某个通道上接收到 EIOS 或者检测到电气空闲状态，并且在进入本状态后未接收到任何 TS2 序列。这种情况表示链路在当前速率下无法工作，解决的办法是把新的速率降低回到最低的 2.5 GT/s。

***转换至 Configuration状态***

如果在任意已配置通道上，接收到 8 个连续的 TS1 序列，其链路编号或者通道编号和发送的数值不一致，并且 speed_change 比特为 0b，或者不共同支持高于 2.5 GT/s 的速率。

进入 Configuration 状态前，内部变量 changed_speed_recovery 和 directed_speed_change 会清除为 0b。如果 N_FTS 变量有更新，那么更新后的数值必须用于后续的 L0s 状态。

***转换至 Detect状态***

如果 48ms 超时后，上述各种状态转换的条件都没有被满足，那么在  2.5 GT/s 或者 5.0 GT/s 速率时，状态会转移为 Detect。

如果速率为  8.0 GT/s，那么还存在一种可能的状态转移行为，那就是虽然本次速率改变失败了，但是速率改变的可尝试次数还没有用完，这一点由变量 idle_to_rlock_transitioned 的数值所反应。如果速率为  8.0 GT/s 时，变量 idle_to_rlock_transitioned 数值小于 FFh，那么新的状态会是 Recovery.Idle。在状态跳转后，变量 changed_speed_recovery 和 directed_speed_change 还是会被清除为 0b。然而，一旦变量 idle_to_rlock_transitioned 数值达到 FFh，那么当 48ms 超时触发后，下一个状态将会是 Detect。
...
***转换至 Disable 状态（Disable State）***

如果以下条件之一满足，那么进入 Disable 状态（Disable State）：

- 链路对端的 DSP 或者，可选支持的 crosslink 端口受到上层逻辑的指令，在所发送的 TS1/TS2 中使能链路关闭（Disable Link）比特。
...
- 链路对端的 DSP 或者，可选支持的 crosslink 端口受到上层逻辑的指令，在所发送的 TS1/TS2 中使能 Hot Reset比特。
...
- 发送端确定自己有成为 Loopback 主机的能力（细节由具体实现决定，协议没有提供具体的方式去验证这一点）并受到上层逻辑的指令，在所发送的 TS1/TS2 中使能 Loopback 比特。
...
发送端在上层逻辑的命令下进入 L0s 状态。协议没有具体规定发送端进入 L0s 状态的条件，但直观地说，发送端会在空闲状态持续一段时间后进入 L0s 状态：这段时间内没有发送过 TLP 或者 DLLP。为了进入 L0s 状态，发送端通过发送一个 EIOS （5.0 GT/s 速率下发送两个 EIOS），进入电气空闲状态。然而，此时发送端还没有真正关闭，并且需要维持共模电压在协议规定的范围内。
...
在任何速率下，不能在 FTS 序列之前发送 SOS 序列。然后，在 5.0 GT/s 速率时，必须在 FTS 序列之前发送 4 到 8 个 EIE 符号。在 128b/130b 编码时，必须在 FTS 序列之前发送一个 EIEOS 序列。
...
如果 extended synch 比特置位，那么最小超时时间是 2048 个 FTS，最大超时时间是 4096 个 FTS。如果是超过 2.5 GT/s 的速率，那么接收端实际用到的超时时间还要加上 4 到 8 个 EIE 符号。
...
当速率超过 2.5 GT/s 时，LTSSM 必须在该次状态至少停留 40ns。在协议中，该延迟被描述为 “用于给逻辑准备好电气空闲状态检测电路留出时间，防止出现链路进入 L1 又很快退出的情况（检测不到这次进出 L1 状态）”。
...
L2 状态的目的在于使用低功耗的 Vaux 给设备供电，在需要链路恢复电源的事件发生后唤醒系统。设备有两种标准方法通知系统该类事件的发生。一个是边带（side-band）信号 WAKE# 引脚，另一个是链路（in-band）信号 Beacon。WAKE# 信号对于 L2 来说不是必须的，但是如果使用可选的 Beacon，那么其是必须的。协议中明确指出工作在 5.0 或者 8.0 GT/s 的设备，无需支持 Beacon 特性，所以看上去 Beacon 是一项过时（legacy）的特性，仅用于运行在 2.5 GT/s 的设备。关于链路唤醒选项的更多细节，可以参见原文 772 页的 “Waking Non-Communicating Links” 一节。
...
### 14.12 热复位状态（Hot Reset State）

端口进入热复位状态（Hot Reset State）通常有两种原因：一种是该端口属于桥设备（Bridge），并且软件通过配置空间触发了向下游传播的热复位（Hot Reset）；另一种是端口连续接收到两个 Hot Reset 位被置位的 TS1 有序集。

#### 14.12.1 热复位期间

在 Hot Reset 状态中，端口会持续发送 Hot Reset 位被置位的 TS1，但不会改变已经配置好的链路编号（Link Number）和通道编号（Lane Number）。

如果交换机的上游端口（USP，Upstream Port）进入 Hot Reset 状态，则该交换机所有已经配置好的下游端口（DSP，Downstream Port）都必须尽快转入 Hot Reset 状态。

#### 14.12.2 转移至 Detect 状态

对于发起 Hot Reset 的桥端口，当软件清除配置空间中触发 Hot Reset 的控制位后，该桥端口会转入 Detect 状态。不过，该端口必须至少在 Hot Reset 状态停留 2 ms。

对于因为连续接收到两个 Hot Reset 位被置位的 TS1 而进入 Hot Reset 的端口，只要它持续接收到这类 TS1，就会继续停留在 Hot Reset 状态。一旦端口不再接收 Hot Reset 位被置位的 TS1，它将转入 Detect 状态。不过，该端口同样必须至少在 Hot Reset 状态停留 2 ms。

### 14.13 Disable 状态（Disable State）

被禁用的链路（Disabled Link）处于电气空闲（Electrical Idle）状态，并且不需要维持直流共模电压（DC common-mode voltage）。软件可以通过设置设备 Link Control 寄存器中的 Link Disable 位来发起该过程，随后设备会发送 Disable Link 位被置位的 TS1。

#### 14.13.1 Disable 期间

所有通道会发送 16 到 32 个 Disable Link 位被置位的 TS1，然后发送一个 EIOS；如果当前速率为 5.0 GT/s，则发送两个连续的 EIOS。之后，链路进入电气空闲状态。此时直流共模电压不需要保持在规范要求的范围内。

如果某端口已经发送了 EIOS（5.0 GT/s 下为两个连续 EIOS），并且在任意已配置通道上也接收到了 EIOS，则 `LinkUp = 0b`（False），这些通道被认为已经禁用。

#### 14.13.2 转移至 Detect 状态

对于上游端口（USP），当接收端检测到电气空闲状态，或者在 2 ms 超时窗口内没有接收到 EIOS 时，下一个状态为 Detect。

对于下游端口（DSP），下一个状态同样为 Detect，但必须等到软件将 Link Disable 位清零为 `0b` 后，才允许转入 Detect。

### 14.14 回环状态（Loopback State）

回环状态（Loopback State）是一种测试和调试特性，正常运行过程中不会使用。作为回环主设备（Loopback Master）的设备，可以通过发送 Loopback 位被置位的 TS1，将链路伙伴置于回环从设备（Loopback Slave）模式。该机制可以在系统板级环境中使用，因此也可用于对链路执行 BIST（Built-In Self Test，内建自测试）。

进入该状态后，Loopback Master 会向 Loopback Slave 发送有效符号，Loopback Slave 随后将这些符号回送。Loopback Slave 仍会执行时钟容差补偿（Clock Tolerance Compensation），因此 Master 必须继续按正确间隔插入 SOS。为了执行时钟容差补偿，Loopback Slave 在回送 SOS 时，可能需要添加或删除 SKP 符号。

当 Loopback Master 发送 EIOS，且接收端检测到电气空闲状态后，回环状态退出。回环状态机包括 Loopback.Entry、Loopback.Active 和 Loopback.Exit 等次状态。

#### 14.14.1 Loopback.Entry

该次状态的典型行为是：Loopback Master 持续发送 Loopback 位被置位的 TS1，直到它开始看到这些 TS1 被回送回来。一旦 Loopback Master 看到带有 Loopback 位的 TS1 被回送，就可以判断链路伙伴已经进入 Loopback Slave 模式，并且正在简单地重复发送它接收到的所有内容。

在该次状态中，链路不被认为处于激活状态，即 `LinkUp = 0b`。同时，接收端会忽略 TS1 和 TS2 中使用的 Link Number 和 Lane Number。规范特别指出，在 128b/130b 编码下使用 Lane Number 时需要注意：每个 Lane 的加扰器（Scrambler）使用不同的种子值。如果在进入 Loopback 模式前尚未完成 Lane Number 协商，链路两端可能会对 Lane 分配产生不同理解，从而无法识别接收到的符号。可以通过以下方式避免该问题：等到 Lane Number 协商完成后再指示 Master 进入 Loopback 状态，或者在 Loopback.Entry 期间指示 Master 置位 Compliance Receive 位，或采用其他实现方式。

#### 14.14.2 Loopback Master

在 Loopback.Entry 次状态中，Loopback Master 会持续发送 Loopback 位被置位的 TS1。Master 也可以在 TS1 中置位 Compliance Receive 位，以便在一端或两端端口在速率改变后难以获得位锁定（Bit Lock）、符号锁定（Symbol Lock）或块对齐（Block Alignment）时辅助测试。如果该位被置位，则在当前状态中不得清除。

如果该次状态是从 Configuration.Linkwidth.Start 进入的，则需要检查当前速率是否为链路双方共同支持的最高速率。如果不是，需要执行以下操作：

- 切换到双方共同支持的最高速率。具体流程是发送 16 个 Loopback 位被置位的 TS1，随后发送一个 EIOS；如果当前速率为 5.0 GT/s，则发送两个 EIOS。随后进入电气空闲状态 1 ms，并在该空闲期间切换到双方共同支持的最高速率。
- 如果双方共同支持的最高速率为 5.0 GT/s，则 Slave 的发送端去加重（Tx De-emphasis）由 Master 在 TS1 中设置 Selectable De-emphasis 位来控制：`1b = -3.5 dB`，`0b = -6 dB`。
- 对于 5.0 GT/s 及更高数据速率，Master 的发送端可以选择任意去加重设置，而不受它发送给 Slave 的设置限制。
- 需要注意一种潜在问题：如果链路已经训练到 L0 且 `LinkUp = 1b` 后再进入 Loopback，则可能出现一个端口从 Recovery 进入 Loopback，而链路伙伴从 Configuration 进入 Loopback 的情况。如果发生这种情况，后者可能尝试改变速率，而前者不会改变速率，结果将处于未定义状态。规范要求测试环境避免这类冲突场景。

**转移至 Loopback.Active**

在以下任一条件满足后，Master 的下一个状态为 Loopback.Active：

- 如果输出 TS1 中置位了 Compliance Receive 位，则经过 2 ms 后进入 Loopback.Active。
- 如果输出 TS1 中未置位 Compliance Receive 位，并且在设计指定数量的 Lane 上连续接收到两个 Loopback 位被置位的 TS1，则进入 Loopback.Active。

如果期间发生过速率切换，Master 必须确保已经发送足够数量的 TS1，使 Slave 能够获得符号锁定或块对齐，然后才能进入 Loopback.Active。

**转移至 Loopback.Exit**

如果进入 Loopback.Active 的条件均未满足，则在小于 100 ms 的、由设计定义的超时时间到达后，下一个状态为 Loopback.Exit。

#### 14.14.3 Loopback Slave

Loopback Slave 通过连续接收到两个 Loopback 位被置位的 TS1 进入该次状态。

如果该次状态是从 Configuration.Linkwidth.Start 进入的，则需要检查当前速率是否为链路双方共同支持的最高速率。如果不是，需要执行以下操作：

- 切换到双方共同支持的最高速率。Slave 发送一个 EIOS；如果当前速率为 5.0 GT/s，则发送两个 EIOS。随后进入电气空闲状态 2 ms，并在该空闲期间切换到双方共同支持的最高速率。
- 如果双方共同支持的最高速率为 5.0 GT/s，则根据接收到的 TS1 中的 Selectable De-emphasis 位设置发送端去加重：`1b = -3.5 dB`，`0b = -6 dB`。
- 如果双方共同支持的最高速率为 8.0 GT/s：如果是 EQ TS1 指示 Slave 进入该状态，则使用其中指定的 Tx Preset 设置；如果是普通 TS1 指示 Slave 进入该状态，则允许 Slave 使用其默认发送端设置。

**转移至 Loopback.Active**

如果指示 Slave 进入该状态的输入 TS1 中置位了 Compliance Receive 位，则下一个状态为 Loopback.Active。此时 Slave 不需要等待特定边界即可发送回环数据，并允许截断正在进行的任意有序集。

否则，Slave 会发送 Link Number 和 Lane Number 均为 PAD 的 TS1；当满足以下条件时，下一个状态为 Loopback.Active：

- 当前速率为 2.5 GT/s 或 5.0 GT/s，并且所有 Lane 都已获得符号锁定。
- 当前速率为 8.0 GT/s，并且在所有活动 Lane 上都检测到两个连续的 TS1。此时均衡通过评估并应用 TS1 中给出的数值完成，前提是这些数值被支持，并且 EC 值适合端口方向：下游端口为 `10b`，上游端口为 `11b`。端口也可以选择在该场景下接受任一 EC 值。如果应用了这些设置，则必须在接收后 500 ns 内生效，并且不得导致发送端在超过 1 ns 的时间内违反任何电气规范。与 Recovery.Equalization 过程相比，一个重要区别是：Slave 发送的 TS1 不会回显新的设置。
- 对于 8b/10b 编码，Slave 必须仅在符号边界上切换到回送数据，但允许截断正在进行的任意有序集。对于 128b/130b 编码，规范没有规定何时可以开始发送回送数据的具体边界，同样允许截断正在进行的任意有序集。

#### 14.14.4 Loopback.Active

在该次状态中，Loopback Master 发送有效的编码数据，并且在准备退出 Loopback 之前不应发送 EIOS。Loopback Slave 会不加修改地回送接收到的信息；即使其编码被判断为无效，也应回送。唯一可能的例外是根据 Polling 状态中确定的极性反转情况进行极性翻转。Slave 还会继续执行时钟容差补偿，这意味着它需要按需添加或删除 SKP，但不要求所有 Lane 添加或删除相同数量的 SKP。

**转移至 Loopback.Exit**

如果 Loopback Master 被指示退出，则其下一个状态为 Loopback.Exit。

对于 Loopback Slave，如果满足以下任一条件，则下一个状态为 Loopback.Exit：

- Slave 被指示退出，或任意 Lane 上检测到四个连续 EIOS。
- 可选地，在当前速率为 2.5 GT/s 时，如果任意 Lane 接收到 EIOS、检测到电气空闲，或推断出电气空闲，也可以进入 Loopback.Exit。如果任意已配置 Lane 在 128 μs 内未检测到电气空闲退出，则可以推断链路处于电气空闲。

Slave 必须能够在接收到 EIOS 后 1 ms 内，在任意 Lane 上检测到电气空闲。在接收到 EIOS 到实际检测到电气空闲之间，Loopback Slave 可能会接收到编码方案未定义的比特流，并可能将其回送给发送端。

#### 14.14.5 Loopback.Exit

在该次状态中，对于只支持 2.5 GT/s 的端口，Loopback Master 会发送一个 EIOS；对于支持高于 2.5 GT/s 速率的端口，Loopback Master 会连续发送八个 EIOS。仅支持 2.5 GT/s 的端口也可以选择发送八个 EIOS。随后，Master 会在所有 Lane 上进入电气空闲状态并保持 2 ms。

Loopback Master 必须在发送最后一个 EIOS 后的 `TTX-IDLE-SET-TO-IDLE` 时间内转入电气空闲状态。需要注意的是，EIOS 标志着 Master 发送和比较操作的结束；在接收到任何 EIOS 后，Master 后续接收到的任何数据都处于未定义状态，应被忽略。

Loopback Slave 必须在所有 Lane 上进入电气空闲状态并保持 2 ms，但在检测到电气空闲之前，它必须回送所有已经接收到的符号，以确保 Master 能够看到 EIOS 的到达，并将其作为逻辑发送和比较操作的结束标志。

**转移至 Detect 状态**

在完成所需 EIOS 交换并且所有 Lane 已经处于电气空闲状态 2 ms 后，下一个状态为 Detect。

### 14.15 动态带宽改变（Dynamic Bandwidth Changes）
...
通过原文 623 页的图 14-49 展示的速率改变示例，我们能更清楚地了解整个过程。注意，该示例中略去了均衡（Equalization）相关状态，以使整个示例的示意图更加简单和容易理解。考虑到示例展示的是从 2.5 GT/s 改变到 5.0 GT/s 的过程，所以反正也不会进入均衡次状态。如果速率要改变为 8.0 GT/s，那么整个过程也是相同的，除了需要在最后增加进入均衡次状态进行均衡的过程。如果需要了解更多关于均衡次状态的信息，可以参照原文 587 页的  “Recovery.Equalization” 一节（译文 14.8.6 节）。
...
**结束阶段**，在进入 Recovery.Speed 次状态后，链路会在两个方向上都被置于电气空闲状态，并调整内部电路的速率。调整后的速率是双方在 TS1 以及 TS2 中发送的支持速率列表（Rate ID 域）中双方同时支持的最高速率。本例中，双方共同支持的最高速率为 5.0 GT/s，所以链路改变为该速率。在等待一段延时后，链路双方同时回到 Recovery.RcvrLock 次状态，并通过重新发送 TS1 退出电气空闲状态，如原文 627 页图 14-55 所示。在 USP 接收到重新发送的 TS1 后，其 LTSSM 跳转到 Recovery.RcvrCfg 状态，开始发送 TS2 序列，同上一次由Recovery.RcvrLock 次状态进入 Recovery.RcvrCfg 次状态过程相同。不过此时 TS1 中的 Speed Change 比特不会被置起。最终，USP 接收到 DSP 返回的，Speed Change 比特未置起的 TS2 序列，此时 USP （以及 DSP）LTSSM 途径 Recovery.Idle 次状态返回 L0 状态。
...
让我来看一个示例，示例基于如原文 631 页图 14-59 所示的连接拓扑，一个根端口（Root Port）连接到一个 EP（千兆以太网设备）。同样地，只有 USP 才能够发起链路宽度的改变，和前文速率改变时一样，USP 先使自身进入 Recovery 状态发起这一操作。但此时不会在 TS1/TS2 中置起 Speed Change 标志。为了确定改变后的链路宽度，USP 会通知 DSP 在从 Recovery 状态返回 L0 状态之前，进入一次 Configuration 状态，如原文 631 页图 14-60 所述。Configuration 状态包含多个次状态，原文 632 页的图 14-60 给出了一个简化版的状态跳变过程示意图。接下来，我们会一一讨论各个步骤。
...
### 14.16 相关配置寄存器（Related Configuration Registers）
...
该只读寄存器表示当前的链路速度。在链路首次进入 L0 状态时，链路速度始终为 2.5 GT/s。在此之后，如果链路双方共同支持更高的速率，则链路通过进入 Recovery 状态，尝试改变到更高的速度。该寄存器域的编码方式和最大链路速度域相同，为一个指向链路支持速率向量域的指针：
...
**注意**：如果链路还没有 LinkUp，则该域为未定义状态（LinkUp = 0b）。
