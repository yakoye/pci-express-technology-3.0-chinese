...
8. Producer 现在有更多数据需要发送，它会通过发起 MRd 事务（Non-Posted 请求）的方式周期性地检查 Status 信号量。
9. Status 信号量被 Producer 读取，并以 MRd Completion 的形式返回给 Producer，用于告知 Producer：Consumer 尚未读取内存缓冲区内容，也尚未更新 Status 信号量（此时 Status = 0）。
10. Consumer 现在已经知道内存缓冲区中有可用数据，因此会发起一个或多个 MRd（Non-Posted 请求），从缓冲区中获取这些数据。
11. 内存缓冲区的内容被读取并返回给 Consumer。
12. 在完成内存到 Consumer 的数据传输后，Consumer 发起一个 MWr（Posted 请求），将 Status 信号量置为 1。
13. Producer 再次通过发起 MRd（Non-Posted 请求）来检查 Status 信号量。
14. Producer 设备读取 Status，此时 Status = 1。MRd Completion 返回给 Producer，从而 Producer 得知内存缓冲区中的数据已经被 Consumer 读出，因此可以继续向内存缓冲区发送数据。
15. Producer 发起 MWr，将 Status 信号量清零。
16. Producer 重复从步骤 1 开始的一系列事件。
