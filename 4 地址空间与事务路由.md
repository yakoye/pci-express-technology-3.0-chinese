## 4 地址空间与事务路由（Address Space & Transaction Routing）
...
Prefetchable Memory Base寄存器：

- 值为4001h。

- 用法为，寄存器的高12bit用来存放32bit BASE地址的高12bit，也就是[31:20]。Base地址中的低20bit全是0，这意味着Base地址总是在1MB边界上对齐。

- 寄存器的低4bit用于指示Bridge中是否支持64bit地址译码，这4bit值为1则意味着Upper Base/Limit寄存器要被启用。

Prefetchable Memory Limit寄存器：

- 值为43F1h。
- 用法与Base寄存器相似，这个寄存器的高12bit用来存放32bit LIMIT地址的高12bit，也就是[31:20]。Limit地址的低20bit为全1（16进制下都是F）。这个寄存器的低4bit和base寄存器的低4bit含义相同。

Prefetchable Memory Base Upper 32bits寄存器：

- 值为00000002h。
- 用法为，这个寄存器用来存放这个端口下方可预取内存的64bit BASE地址的高32bit。

Prefetchable Memory Limit Upper 32bits寄存器：

- 值为00000002h。
- 用法为，这个寄存器用来存放这个端口下方可预取内存的64bit Limit地址的高32bit。
...
Non-Prefetchable Memory Base寄存器：
...
Non-Prefetchable Memory Limit寄存器：
...
- 用法为，**寄存器的高4bit**用来存放16bit BASE地址的高4bit，也就是[15:12]。Base地址中的低12bit全部被设置为0，这意味着Base地址总是在4KB边界上对齐。
...
- 用法与Base寄存器相似，这个寄存器的高4bit用来存放16bit LIMIT地址的高4bit，也就是[15:12]。Limit地址的低12bit为全1（16进制下都是F）。这个寄存器的低4bit和base寄存器的低4bit含义相同。

IO Base Upper 32bits寄存器：

- 值为0000h。

- 用法为，这个寄存器用来存放这个端口下方IO地址空间的32bit BASE地址的高16bit。

IO Limit Upper 32bits寄存器：

- 值为0000h。

- 用法为，这个寄存器用来存放这个端口下方IO地址空间的32bit Limit地址的高16bit。
...
- **Memory Read**和**Memory Read Lock**：
...
- **Configuration Read**和**Configuration Write**：
...
###### 4.6.2.3.1 TLP向下行移动（由Switch主接口接收并向下转发）
...
###### 4.6.2.3.2 TLP向上行移动（由次级接口接收并向上转发）
